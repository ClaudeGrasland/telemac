...
testchi2<-function(tabtest=tabtest,
  ...
  tab$signif[is.na(tab$signif)]<-paste("est. succ. <",mintest)
                      ...
                      title = "Who says What ?")
{
  ...
  p <- plot_ly(tab,
               ...
               text = ~paste('Source: ',who,
                             ...
                             '<br /> Salience (% of topic news) : ', round(estimate*100,2),'%',
                             '<br /> Reference : ', round(null.value*100,2),'%',
                             '<br /> p.value & test: ', round(p.value,4),' ',signif),
               type = "bar")  %>%
    layout(title = title,
                           ...
                           title = "What by Whom and When ?")
{
  ...
  p <- plot_ly(tab,
               ...
               text = ~paste('Date: ',when,
                             ...
                             '<br /> Topic news : ', round(success,0),
                             '<br /> Salience (% of topic news) : ', round(estimate*100,2),'%',
                             '<br /> Reference (source) : ', round(null.value*100,2),'%',
                             '<br /> p.value & test: ', round(p.value,4),' ',signif
               ),
               #  name = ~tags,
               type = "heatmap")  %>%
                             ...
                             title = "What, Where and When ?")
{
  ...
  p <- plot_ly(tab,
               ...
               text = ~paste('Location: ',where1,
                             ...
                             '<br /> Topic news : ', round(success,0),
                             '<br /> Salience (% of topic news) : ', round(estimate*100,2),'%',
                             '<br /> Reference (country) : ', round(null.value*100,2),'%',
                             '<br /> p.value & test: ', round(p.value,4),' ',signif
               ),
               #  name = ~tags,
               type = "heatmap")  %>%
